...
def transpile(ir_node_list):
    ...
    name_map = {"ADDI": "addi","SUBI": "subi", "MULTI": "muli",  "DIVI": "divi", "ADDF": "addr","SUBF": "subr", "MULTF": "mulr",  "DIVF": "divr",
                ...
                "READI": "sys readi","READF": "sys readr","WRITEI": "sys writei","WRITEF": "sys writer",
                "STOREI": "move", "STOREF":"move", "WRITEI": "sys writei","RET": "sys halt"}
    var_stack = Stack.Stack()
    seen_var_names = []
    for ir_node in ir_node_list:
        if ir_node.op_code in ["GTI","GEI","LTI","LEI","NEI","EQI","GTF","GEF","LTF","LEF"]:
            new_op1, new_op2 = new_op(ir_node.op1, ir_node.op2)
            node0 = Node.LittleNode("cmpr",new_op1, new_op2)
            node1 = Node.LittleNode(name_map[ir_node.op_code],ir_node.op3)
            ...
            node = Node.LittleNode("jmp", ir_node.op2)
            node_list.append(node)
        elif ir_node.op_code in ["LABEL"]:
            node = Node.LittleNode("label", ir_node.op2)
            node_list.append(node)
        elif ir_node.op_code in ["READI","READF","WRITEI","WRITEF"]:
